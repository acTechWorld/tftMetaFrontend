<template>
  <div ref="tileRef">
    <div
      class="flex flex-col items-center md:gap-1 touch-none"
      @dragover="onDragOver"
    ...
    >
      ...
      >
        ...
        <img
          v-if="main"
          :src="main.src"
          class="absolute inset-0 w-full h-full object-cover touch-none"
        ...
        <template v-for="(sub, i) in subs" :key="i" >
          ...
          >
            <img
              v-if="sub"
              :src="sub.src"
              class="w-full h-full object-cover touch-none"
              draggable="true"
...
const { mobileDragItem, mobileDragPos, endMobileDrag, startMobileDrag, updateMobileDrag } = useMobileDrag();
...
onMounted(() => {
  ...
  window.addEventListener("touchend", onMobileDrop);
});
onUnmounted(() => {
  window.removeEventListener('keydown', onKeyDown);
  window.removeEventListener('keyup', onKeyUp);
   window.removeEventListener("touchend", onMobileDrop);
});
...
function onMobileDrop() {
  if (!mobileDragItem.value) return;
  if (!tileRef.value) return;

  const rect = tileRef.value.getBoundingClientRect();
  const { x, y } = mobileDragPos.value;
  ...
  const item = endMobileDrag();
  if (!item) return;

  if (item.type === "champion") emits("drop-main", item);
  else emits("drop-sub", item);
}
